#  I'm jut gonna start making other people's tutorials and 
#  then add updates where needed.
#  

#  Clustering Frankenstein
#  Antonio Sánchez Chinchón
#  https://fronkonstin.com/2019/08/05/clustering-frankenstein/
...
# Let's try with this stock image of a lightbulb for 11/7 "light"
# https://images.pexels.com/photos/577514/pexels-photo-577514.jpeg?cs=srgb&dl=bulb-close-up-electricity-577514.jpg&fm=jpg
# I just google image searched "stock light bulb"


# fronkonstin code:

# Location of the photograph
lightbulb <- "lightbulb-close-up-electricity-577514.jpg"

# Load, convert to grayscale, filter image (to convert it to bw) and sample
load.image(lightbulb) %>% 
  grayscale() %>% 
  threshold("45%") %>% 
  as.cimg() %>% 
  as.data.frame() -> light
...
clustResultx <- function(x) {
  clustCut <- tibble(cluster_id = cutree(clusters, x)) %>% 
              bind_cols(data)
  clustCut %>% 
    group_by(cluster_id) %>% 
    summarize(size = n()) %>% 
...
## final image is a little sparse and it 
## only shows the background, not the detailing of the coiling. 
## which makes sense but I want it to be more fine tuned.
## 
## decent first start
## next version will be more complex or dense. Pretty neato.